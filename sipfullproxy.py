    ...
    def processInvite(self):
        ...
        if len(destination) > 0:
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                showtime()
                for x in data:
                    if str(x).split(" ")[0] == "Call-ID:":
                        call_id = x.split()[1]
                        break
                if call_id not in calls:
                    logging.info(time.strftime("%a, %d %b %Y %H:%M:%S ",
                                 time.localtime()))
                    logging.info("CALL [" + call_id +
                                 "] FROM " + self.getOrigin()
                ...
                logging.info(destination + " 480 Temporarily Unavailable")
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                for x in data:
                    if str(x).split()[0] == "Call-ID:":
                        call_id = x.split()[1]
                        break
                if str(data[0].split()[0]) == "BYE":
                    logging.info(time.strftime("%a, %d %b %Y %H:%M:%S ",
                                               time.localtime()))
                    logging.info("CALL [" + call_id + "] FINISHED")
                    calls.remove(call_id)
                    running.remove(call_id)
                elif str(data[0].split()[0]) == "CANCEL":
                    logging.info(time.strftime("%a, %d %b %Y %H:%M:%S ",
                                               time.localtime()))
                    logging.info("CALL [" + call_id + "] CANCELED")
                # logging.info("<<< %s" % data[0])
                # logging.debug(
                #     "---\n<< server send [%d]:\n%s\n---" % (len(text), text))
            else:
    ...
    def processCode(self):
        ...
        if len(origin) > 0:
            ...
            if origin in registrar:
                ...
                socket.sendto(text.encode(), claddr)
                showtime()
                # logging.info("<<< %s" % data[0].split()[1])
                for x in data:
                    if str(x).split()[0] == "Call-ID:":
                        call_id = x.split()[1]
                        break
                if str(data[0].split()[1]) == "200" and call_id in calls:
                    if call_id not in running:
                        logging.info(time.strftime("%a, %d %b %Y %H:%M:%S ",
                                                   time.localtime()))
                        logging.info("CALL [" + call_id + "] ACCEPTED")
                        running.append(call_id)
                if str(data[0].split()[1]) in ["486", "603"]:
                    logging.info("USER IS BUSY, CALL [" + call_id + "] DENIED")
                    calls.remove(call_id)
                # logging.info(
                #     "---\n<< server send [%d]:\n%s\n---" % (len(text), text))
